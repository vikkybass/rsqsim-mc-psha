"""
Enhanced Mid Angeles Configuration with Visualization Options

Mid Angeles/Central Valley region using exact polygon + Visualizations
"""
...
def load_config(mode: str = None):
    """
    Load Enhanced mid angeles configuration with GUARANTEED directory creation
    ...
    logger.info(f"🚀 Loading mid angeles config for {mode} mode")
    ...
    paths = generate_standardized_paths('mid_angeles', mode)
    ...
    mid_angeles_config = {
        "region": {
            "name": "mid_angeles",
            "description": "mid angeles Bay Area using exact polygon + Guaranteed Directories",
        ...
        "major_cities": {'Fresno': (-119.77, 36.75), 'Bakersfield': (-119.02, 35.37), 'Modesto': (-120.99, 37.64), 'Stockton': (-121.29, 37.96), 'Merced': (-120.48, 37.3), 'Visalia': (-119.29, 36.33), 'Tulare': (-119.35, 36.21), 'Turlock': (-120.85, 37.49), 'Madera': (-120.06, 36.96), 'Hanford': (-119.65, 36.33)}
    }
    
    # Merge configurations
    final_config = base_config.copy()
    final_config.update(mid_angeles_config)
    ...
    logger.info(f"✅ mid angeles configuration loaded successfully for {mode} mode")
...
def validate_config():
    """Validate mid angeles configuration with directory checks"""
    
    logger.info("🔍 Validating mid angeles configuration...")
    ...
    # Report results
    if issues:
        logger.error("❌ mid angeles configuration validation issues:")
        for issue in issues:
            logger.error(f"   • {issue}")
        return False
    
    if warnings:
        logger.warning("⚠️  mid angeles configuration warnings:")
        for warning in warnings:
            logger.warning(f"   • {warning}")
    
    logger.info("✅ mid angeles configuration validation passed")
    return True
...
if __name__ == "__main__":
    ...
    print("Enhanced mid angeles Configuration with Directory Creation")
    print("=" * 80)
    ...
    try:
        ...
        if test_success and is_valid:
            print(f"\n🚀 READY TO RUN:")
            print(f"   ./scripts/run_on_cluster.sh mid_angeles sequential")
            print(f"   ./scripts/run_on_cluster.sh mid_angeles random")
