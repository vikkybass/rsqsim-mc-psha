"""
Enhanced Full California Configuration with Visualization Options

Full California catalog using exact polygon + Visualizations
"""
...
def load_config(mode: str = None):
    """
    Load Enhanced Full california configuration with GUARANTEED directory creation
    ...
    logger.info(f"🚀 Loading Full california config for {mode} mode")
    ...
    paths = generate_standardized_paths('Full_california', mode)
    ...
    Full_california_config = {
        "region": {
            "name": "Full_california",
            "description": "Full california Bay Area using exact polygon + Guaranteed Directories",
        ...
        "hazard_calculation": {
            "method": "c_style",  # Use C-style method or high_resolution
        },
        
        "output_settings": {
            "full_output": False,
            "min_ground_motion": 0.05,
            "probability_type": "non_exceedance",
            "probabilities": [[50, 0.98], [50, 0.95], [50, 0.90]],
            "plot_hazard_curves": False,
            "max_curves": 0,
            "export_gis_csv": True,
        ...
        "resource_limits": {
            "max_memory_gb": 32,
            "batch_size": 200,
            "max_events_per_site": 50000,
            "max_synthetic_events": 20000
        },
        "performance": {
        ...
        "major_cities": {'Los Angeles': (-118.25, 34.05), 'San Francisco': (-122.42, 37.77), 'San Diego': (-117.16, 32.72), 'Sacramento': (-121.49, 38.56), 'Fresno': (-119.77, 36.75), 'Oakland': (-122.27, 37.8), 'Long Beach': (-118.19, 33.77), 'Bakersfield': (-119.02, 35.37)}
    }
    
    # Merge configurations
    final_config = base_config.copy()
    final_config.update(Full_california_config)
    ...
    logger.info(f"✅ Full california configuration loaded successfully for {mode} mode")
...
def validate_config():
    """Validate Full california configuration with directory checks"""
    
    logger.info("🔍 Validating Full california configuration...")
    ...
    # Report results
    if issues:
        logger.error("❌ Full california configuration validation issues:")
        for issue in issues:
            logger.error(f"   • {issue}")
        return False
    
    if warnings:
        logger.warning("⚠️  Full california configuration warnings:")
        for warning in warnings:
            logger.warning(f"   • {warning}")
    
    logger.info("✅ Full california configuration validation passed")
    return True
...
if __name__ == "__main__":
    ...
    print("Enhanced Full california Configuration with Directory Creation")
    print("=" * 80)
    ...
    try:
        ...
        if test_success and is_valid:
            print(f"\n🚀 READY TO RUN:")
            print(f"   ./scripts/run_on_cluster.sh Full_california sequential")
            print(f"   ./scripts/run_on_cluster.sh Full_california random")
