...
Central Coast (Santa Barbara to Monterey) using exact polygon + Visualizations
"""
...
def load_config(mode: str = None):
    """
    Load Enhanced central coast configuration with GUARANTEED directory creation
    ...
    logger.info(f"🚀 Loading central coast config for {mode} mode")
    ...
    paths = generate_standardized_paths('central_coast', mode)
    ...
    central_coast_config = {
        "region": {
            "name": "central_coast",
            "description": "central coast Bay Area using exact polygon + Guaranteed Directories",
        ...
        "major_cities": {'Santa Barbara': (-119.7, 34.42), 'San Luis Obispo': (-120.66, 35.28), 'Monterey': (-121.9, 36.61), 'Salinas': (-121.45, 36.33), 'Paso Robles': (-120.85, 35.37), 'Santa Maria': (-120.44, 34.95), 'Lompoc': (-120.46, 34.64), 'Morro Bay': (-120.85, 35.37), 'Carmel': (-121.92, 36.55), 'Seaside': (-121.85, 36.61)}
    }
    
    # Merge configurations
    final_config = base_config.copy()
    final_config.update(central_coast_config)
    ...
    logger.info(f"✅ central coast configuration loaded successfully for {mode} mode")
...
def validate_config():
    """Validate central coast configuration with directory checks"""
    
    logger.info("🔍 Validating central coast configuration...")
    ...
    # Report results
    if issues:
        logger.error("❌ central coast configuration validation issues:")
        for issue in issues:
            logger.error(f"   • {issue}")
        return False
    
    if warnings:
        logger.warning("⚠️  central coast configuration warnings:")
        for warning in warnings:
            logger.warning(f"   • {warning}")
    
    logger.info("✅ central coast configuration validation passed")
    return True
...
if __name__ == "__main__":
    ...
    print("Enhanced central coast Configuration with Directory Creation")
    print("=" * 80)
    ...
    try:
        ...
        if test_success and is_valid:
            print(f"\n🚀 READY TO RUN:")
            print(f"   ./scripts/run_on_cluster.sh central_coast sequential")
            print(f"   ./scripts/run_on_cluster.sh central_coast random")
