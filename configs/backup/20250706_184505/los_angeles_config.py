"""
los_angeles grid Site Configuration with Visualization Options
"""
...
def load_config():
    ...
    region_bounds = get_region_bounds('los_angeles')
    
    # Enhanced los_angeles configuration
    la_config = {
        "region": {
            "name": "los_angeles",
            "description": "Los Angeles region using exact polygon from catalog filtering + Visualizations",
        ...
        "paths": {
            ...
            "windows_dir": f"{RSQSIM_WORK_DIR}/data/Catalog_4983/windows/los_angeles/sequential",
            "output_dir": f"{RSQSIM_WORK_DIR}/output/los_angeles/sequential/multi_site", 
            "log_dir": f"{RSQSIM_WORK_DIR}/logs/los_angeles/multi_site",
            
            # Backup directories
            "backup_dir": f"{RSQSIM_PROJECT_DIR}/rsqsim_results/los_angeles/multi_site",
            "archive_dir": f"{RSQSIM_PROJECT_DIR}/rsqsim_archive/los_angeles/multi_site",
        },
        ...
        "site": {
            ...
            # For testing (coarse):
            "grid_lat_spacing": 0.05,   # ~55km, creates ~483 sites
            "grid_lon_spacing": 0.05,
            ...
            "sites": [(34.05, -118.25)],  # Downtown LA (within the polygon)
        ...
        "output_settings": {
            ...
            "min_ground_motion": 0.005,  # Same as site setting
            ...
            "color_scheme": "plasma",    # Color scheme for maps
        ...
        # Major cities in LA region for map annotation
        "major_cities": {
...
def filter_sites_to_region(sites, region_name='los_angeles'):
    """
...
def load_multi_site_config():
    ...
    config["site"]["grid_lat_min"] = 33.7
    config["site"]["grid_lat_max"] = 34.3
    config["site"]["grid_lon_min"] = -118.8
    config["site"]["grid_lon_max"] = -117.6
    config["site"]["grid_lat_spacing"] = 0.1  # 4 points
    config["site"]["grid_lon_spacing"] = 0.1  # 6 points
...
def validate_config():
    ...
    if issues:
        print("Los Angeles configuration validation issues:")
        for issue in issues:
            print(f"  ⚠️  {issue}")
        return False
    
    print("✅ Los Angeles configuration validation passed")
    return True
...
if __name__ == "__main__":
    ...
    if is_valid:
        print(f"\n🚀 READY TO RUN:")
        print(f"python scripts/run_mc.py --region los_angeles --mode sequential --log-level INFO")
    else:
