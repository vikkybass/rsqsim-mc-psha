"""
Enhanced Mid Angeles (Central Valley) Configuration with Visualization Options

This configuration covers the Central Valley region between LA and SF,
including Fresno, Bakersfield, and surrounding areas.
"""
...
def load_config():
    """Load Enhanced Mid Angeles (Central Valley) configuration with visualization options"""
    
    # Get base GMPE configuration
    base_config = load_gmpe_config()
    
    # Get the EXACT bounds used for catalog filtering
    region_bounds = get_region_bounds('mid_angeles')
    
    # Mid Angeles specific configuration
    ma_config = {
        "region": {
            "name": "mid_angeles",
            "description": "Mid Angeles/Central Valley region using exact polygon + Visualizations",
        ...
        "paths": {
            ...
            # Mid Angeles data directories
            "windows_dir": f"{RSQSIM_WORK_DIR}/data/Catalog_4983/windows/mid_angeles/sequential",
            "output_dir": f"{RSQSIM_WORK_DIR}/output/mid_angeles/sequential", 
            "log_dir": f"{RSQSIM_WORK_DIR}/logs/mid_angeles/multi_site",
            
            # Backup directories
            "backup_dir": f"{RSQSIM_PROJECT_DIR}/rsqsim_results/mid_angeles/multi_site",
            "archive_dir": f"{RSQSIM_PROJECT_DIR}/rsqsim_archive/mid_angeles/multi_site",
        },
        ...
        "site": {
            ...
            # Grid resolution for Central Valley
            "grid_lat_spacing": 0.1,   # ~11km spacing
            "grid_lon_spacing": 0.1,
            
            # Key Central Valley sites
            "sites": [
                (36.75, -119.77),  # Fresno
                (35.37, -119.02),  # Bakersfield
                (37.64, -120.99),  # Modesto
                (37.30, -120.48),  # Merced
                (36.33, -119.29)   # Visalia
            ],
            ...
            # Central Valley optimized settings
            "max_distance_km": 300.0  # Larger distances due to lower seismicity
        },
        
        "output_settings": {
            "full_output": False,
            "min_ground_motion": 0.02,  # Higher threshold for lower seismicity region
            ...
            "plot_dpi": 300,
            "plot_style": "seaborn",
            "color_scheme": "YlOrRd",  # Good for inland valleys
        ...
        # Major Central Valley cities for mapping
        "major_cities": {
            "Fresno": (-119.77, 36.75),
            "Bakersfield": (-119.02, 35.37),
            "Modesto": (-120.99, 37.64),
            "Stockton": (-121.29, 37.96),
            "Merced": (-120.48, 37.30),
            "Visalia": (-119.29, 36.33),
            "Tulare": (-119.35, 36.21),
            "Turlock": (-120.85, 37.49),
            "Madera": (-120.06, 36.96),
            "Hanford": (-119.65, 36.33)
        }
    }
    
    # Merge configurations
    final_config = base_config.copy()
    final_config.update(ma_config)
...
def filter_sites_to_region(sites, region_name='mid_angeles'):
    """Filter sites to Mid Angeles polygon"""
    polygon = REGIONAL_POLYGONS[region_name]['region_polygon']
    filtered_sites = []
...
def load_single_site_config():
    """Single site configuration for Fresno"""
    config = load_config()
    config["site"]["grid_mode"] = False
    config["site"]["sites"] = [(36.75, -119.77)]  # Fresno
    config["paths"]["output_dir"] = config["paths"]["output_dir"].replace("sequential", "single_site_test")
    return config

def load_multi_site_config():
    """Multi-site configuration for core Central Valley"""
    config = load_config()
    
    # Focus on Fresno-Bakersfield corridor
    config["site"]["grid_mode"] = True
    config["site"]["grid_lat_min"] = 35.2
    config["site"]["grid_lat_max"] = 37.0
    config["site"]["grid_lon_min"] = -120.5
    config["site"]["grid_lon_max"] = -118.8
    config["site"]["grid_lat_spacing"] = 0.2
    config["site"]["grid_lon_spacing"] = 0.2
    
    return config

def validate_config():
    """Validate Mid Angeles configuration"""
    config = load_config()
    issues = []
    ...
    if issues:
        print("Mid Angeles configuration validation issues:")
        for issue in issues:
            print(f"  ⚠️  {issue}")
        return False
    
    print("✅ Mid Angeles configuration validation passed")
    return True
...
if __name__ == "__main__":
    ...
    if is_valid:
        print(f"\n🚀 READY TO RUN:")
        print(f"python scripts/run_mc.py --region mid_angeles --mode sequential --log-level INFO")
    else:
