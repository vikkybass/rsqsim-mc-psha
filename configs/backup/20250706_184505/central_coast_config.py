"""
Enhanced Central Coast Configuration with Visualization Options

This configuration covers the Central California Coast region including
Santa Barbara, San Luis Obispo, and Monterey areas.
"""
...
def load_config():
    """Load Enhanced Central Coast configuration with visualization options"""
    
    # Get base GMPE configuration
    base_config = load_gmpe_config()
    
    # Get the EXACT bounds used for catalog filtering
    region_bounds = get_region_bounds('central_coast')
    
    # Central Coast specific configuration
    cc_config = {
        "region": {
            "name": "central_coast",
            "description": "Central Coast (Santa Barbara to Monterey) using exact polygon + Visualizations",
        ...
        "paths": {
            ...
            # Central Coast data directories
            "windows_dir": f"{RSQSIM_WORK_DIR}/data/Catalog_4983/windows/central_coast/sequential",
            "output_dir": f"{RSQSIM_WORK_DIR}/output/central_coast/sequential", 
            "log_dir": f"{RSQSIM_WORK_DIR}/logs/central_coast/multi_site",
            
            # Backup directories
            "backup_dir": f"{RSQSIM_PROJECT_DIR}/rsqsim_results/central_coast/multi_site",
            "archive_dir": f"{RSQSIM_PROJECT_DIR}/rsqsim_archive/central_coast/multi_site",
        },
        
        "site": {
            "grid_mode": True,
            
            # Central Coast bounds
            ...
            # Grid resolution for coastal region
            "grid_lat_spacing": 0.1,   # ~9km spacing
            "grid_lon_spacing": 0.1,
            
            # Key Central Coast sites
            "sites": [
                (34.42, -119.70),  # Santa Barbara
                (35.28, -120.66),  # San Luis Obispo
                (36.61, -121.90),  # Monterey
                (35.37, -120.85),  # Paso Robles
                (36.33, -121.45)   # Salinas
            ],
            ...
            # Central Coast optimized settings
            "max_distance_km": 250.0
        },
        
        "output_settings": {
            "full_output": False,
            "min_ground_motion": 0.015,  # Slightly higher for coastal areas
            ...
            "plot_dpi": 300,
            "plot_style": "seaborn",
            "color_scheme": "terrain",  # Good for coastal regions
        ...
        # Major Central Coast cities for mapping
        "major_cities": {
            "Santa Barbara": (-119.70, 34.42),
            "San Luis Obispo": (-120.66, 35.28),
            "Monterey": (-121.90, 36.61),
            "Salinas": (-121.45, 36.33),
            "Paso Robles": (-120.85, 35.37),
            "Santa Maria": (-120.44, 34.95),
            "Lompoc": (-120.46, 34.64),
            "Morro Bay": (-120.85, 35.37),
            "Carmel": (-121.92, 36.55),
...
def filter_sites_to_region(sites, region_name='central_coast'):
    """Filter sites to Central Coast polygon"""
    polygon = REGIONAL_POLYGONS[region_name]['region_polygon']
    filtered_sites = []
...
def load_single_site_config():
    """Single site configuration for Santa Barbara"""
    config = load_config()
    config["site"]["grid_mode"] = False
    config["site"]["sites"] = [(34.42, -119.70)]  # Santa Barbara
    config["paths"]["output_dir"] = config["paths"]["output_dir"].replace("sequential", "single_site_test")
    return config

def load_multi_site_config():
    """Multi-site configuration for core Central Coast"""
    config = load_config()
    
    # Focus on Santa Barbara to San Luis Obispo region
    config["site"]["grid_mode"] = True
    config["site"]["grid_lat_min"] = 34.3
    config["site"]["grid_lat_max"] = 35.5
    config["site"]["grid_lon_min"] = -121.0
    config["site"]["grid_lon_max"] = -119.5
    config["site"]["grid_lat_spacing"] = 0.15
    config["site"]["grid_lon_spacing"] = 0.15
    
    return config

def validate_config():
    """Validate Central Coast configuration"""
    config = load_config()
    issues = []
    ...
    if issues:
        print("Central Coast configuration validation issues:")
        for issue in issues:
            print(f"  ⚠️  {issue}")
        return False
    
    print("✅ Central Coast configuration validation passed")
    return True
...
if __name__ == "__main__":
    ...
    if is_valid:
        print(f"\n🚀 READY TO RUN:")
        print(f"python scripts/run_mc.py --region central_coast --mode sequential --log-level INFO")
    else:
