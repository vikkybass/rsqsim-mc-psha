...
from src.unified_gmpe import log_a, log_a_ensemble
...
def get_gmvalue_func(config):
    """
    Returns a function for ground motion calculations with model-specific handling
    
    Args:
        config: Configuration dictionary containing GMPE settings
        
    Returns:
        Function that calculates ground motion values for given magnitude and distance
    """
    # Verify required parameters exist
    if 'default_model' not in config:
        logger.error("Missing 'default_model' in configuration")
        raise ValueError("Missing 'default_model' in configuration")
    
    # Get the model name for reference
    model_name = config["default_model"]
    
    # Get the model file path if available
    model_file = None
    if 'gmpe_files' in config and model_name in config["gmpe_files"]:
        model_file = config["gmpe_files"][model_name]
    else:
        logger.error(f"Model file for '{model_name}' not found in configuration")
        raise ValueError(f"Model file for '{model_name}' not found in configuration")
    ...
    def gmvalue_func(mag, distance):
        if model_name == "CB14":
            return log_a(
                ...
                distance,
                model=model_name,
                vs30=config.get("vs30", 760.0),  # Add vs30
                period=config.get("default_period", 0.01)  # Add period
            )
        elif model_name in ["Atkinson1995", "Toro1997", "BooreAtkinson1987", "ToroMcGuire1987"]:
            # These models also don't use period
            return log_a(
                mag, 
                distance,
                model=model_name
            ...
            # BSSA14 and other NGAWest2 GMPEs use period
            return log_a(
                mag, 
                distance,
                model=model_name,
                period=config.get("default_period", 0.01),  # Default to 0.01s if not specified
                vs30=config.get("vs30", 760.0)  # Default to 760 m/s if not specified
            )
    
    # Add model name as an attribute of the function for reference
    gmvalue_func.model_name = model_name
